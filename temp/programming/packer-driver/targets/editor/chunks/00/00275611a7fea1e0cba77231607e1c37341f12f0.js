      ...
      _export("MonetizationManager", MonetizationManager = (_dec = ccclass('MonetizationManager'), _dec(_class = class MonetizationManager extends Component {
        ...
        regenerateEnergy() {
          ...
          if (hoursSinceLastRegen >= 1.0) {
            let regenAmount = Math.floor(hoursSinceLastRegen * energyData.regenRate); // VIP加成
        ...
        applyPurchaseReward(item) {
          const value = item.value; // 处理不同类型的奖励
        ...
        applyAdReward(reward) {
          if (reward.type !== 'item' && reward.amount) {
            let amount = reward.amount; // 广告奖励可能有加成
        ...
        loadPlayerData() {
          ...
          if (data) {
            const parsed = JSON.parse(data); // 恢复货币数据
...
//# sourceMappingURL=00275611a7fea1e0cba77231607e1c37341f12f0.js.map