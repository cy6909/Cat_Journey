    ...
    execute: function () {
      ...
      _export("DynamicLevelGenerator", DynamicLevelGenerator = (_dec = ccclass('DynamicLevelGenerator'), _dec(_class = (_class2 = (_class3 = class DynamicLevelGenerator extends Component {
        ...
        generateSpiralLayout(params) {
          const layout = [];
          const baseHealth = this.getBaseHealthForDifficulty(params.difficultyTier);
          const grid = Array(this.gridHeight).fill(null).map(() => Array(this.gridWidth).fill(false));
          let row = 0,
              col = 0;
          let dr = 0,
              dc = 1; // 开始向右移动
        ...
        generateFortressLayout(params) {
          const layout = [];
          const baseHealth = this.getBaseHealthForDifficulty(params.difficultyTier); // 外墙
          ...
          for (let row = 0; row < this.gridHeight; row++) {
            for (let col = 0; col < this.gridWidth; col++) {
              ...
              if (isWall && !isGate) {
                const position = this.getGridPosition(row, col);
                const brickType = (_crd && BrickType === void 0 ? (_reportPossibleCrUseOfBrickType({
                  error: Error()
        ...
        generateChaosLayout(params) {
          const layout = [];
          const baseHealth = this.getBaseHealthForDifficulty(params.difficultyTier); // 随机分布，但确保有一定的可玩性
        ...
        selectBrickType(params, row, col) {
          ...
          const random = Math.random(); // 根据难度和位置选择砖块类型
        ...
        generateFallbackLayout() {
          const layout = [];
          const baseHealth = 1; // Generate a simple 4x6 grid as fallback
...
//# sourceMappingURL=251c813e1550e7b10c84b7afc57c370bac9a3eb2.js.map