  ...
  return {
    ...
    execute: function () {
      ...
      _export("AdvancedEffectSystem", AdvancedEffectSystem = (_dec = ccclass('AdvancedEffectSystem'), _dec(_class = (_class2 = (_class3 = class AdvancedEffectSystem extends Component {
        ...
        addEffectToObject(targetNode, effectType, duration = -1, stackLevel = 1) {
          const objectId = targetNode.uuid;
          const effectId = `${effectType}_${Date.now()}_${Math.random().toString(36).substr(2, 5)}`; // 检查是否启用特效叠加
        ...
        createParticleEffect(effectNode, config, stackLevel) {
          const particle = effectNode.addComponent(ParticleSystem2D); // 基础粒子配置
        ...
        createGlowEffect(effectNode, config, stackLevel) {
          ...
          const uiTransform = effectNode.addComponent(UITransform); // 创建发光贴图
        ...
        createShockwaveEffect(effectNode, stackLevel) {
          ...
          const uiTransform = shockwaveNode.addComponent(UITransform); // 创建环形贴图
        ...
        addFireArmorToPaddle(paddleNode, duration = -1) {
...
//# sourceMappingURL=766094ba93916d4320ed11060a1c4df769b74e7f.js.map