    ...
    execute: function () {
      ...
      _export("SnowMountainBackground", SnowMountainBackground = (_dec = ccclass('SnowMountainBackground'), _dec(_class = (_class2 = class SnowMountainBackground extends Component {
        ...
        createMountainGradientTexture(width, height) {
          ...
          for (let y = 0; y < h; y++) {
            const ratio = y / h; // 雪山渐变：上方深蓝（夜空），下方雪白（雪地）
        ...
        createSnowflakeParticles(transform) {
          ...
          layerTransform.setContentSize(transform.width, transform.height);
          layerTransform.setAnchorPoint(0.5, 0.5);
          const particleSystem = particleNode.addComponent(ParticleSystem2D); // 配置雪花飘落效果
        ...
        createSnowflakeTexture() {
          ...
          const data = new Uint8Array(size * size * 4); // 绘制六角雪花形状
          ...
          for (let y = 0; y < size; y++) {
            for (let x = 0; x < size; x++) {
              ...
              const dx = x - centerX;
              const dy = y - centerY; // 创建十字形和对角线的雪花图案
        ...
        createSingleAurora(transform, index) {
          ...
          const colors = [new Color(50, 255, 50, 100), // 绿色
        ...
        createAuroraTexture() {
          ...
          const data = new Uint8Array(w * h * 4); // 创建波浪形极光纹理

          for (let y = 0; y < h; y++) {
            for (let x = 0; x < w; x++) {
              const index = (y * w + x) * 4; // 使用正弦波创建极光形状
        ...
        startAuroraAnimation(auroraNode) {
          const originalPos = auroraNode.position.clone(); // 缓慢飘动
        ...
        createSingleIcicle(transform, index) {
          ...
          const x = index % 2 === 0 ? -transform.width * 0.4 + Math.random() * 50 : transform.width * 0.4 - Math.random() * 50;
        ...
        createIcicleTexture() {
          ...
          const data = new Uint8Array(w * h * 4); // 创建锥形冰柱
        ...
        createSnowCatTexture() {
          ...
          const data = new Uint8Array(size * size * 4); // 简化的雪地猫咪像素艺术（白猫+蓝色装饰）

          for (let y = 0; y < size; y++) {
            for (let x = 0; x < size; x++) {
              const index = (y * size + x) * 4; // 基础形状：白猫轮廓
...
//# sourceMappingURL=c8fe59761bf8fde1f8ca64b6ffe19acf2e29327b.js.map