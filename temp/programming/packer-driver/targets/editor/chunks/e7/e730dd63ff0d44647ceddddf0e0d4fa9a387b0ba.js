    ...
    execute: function () {
      ...
      _export("ProceduralLevelGenerator", ProceduralLevelGenerator = (_dec = ccclass('ProceduralLevelGenerator'), _dec2 = property({
        ...
        generateNormalLevel(difficulty) {
          const brickCount = Math.min(this.gridWidth * this.gridHeight, Math.floor(20 + difficulty * 8));
          return {
        ...
        generateEliteLevel(difficulty) {
          const brickCount = Math.min(this.gridWidth * this.gridHeight, Math.floor(30 + difficulty * 10));
          return {
        ...
        generateBrickPositions(count) {
          const positions = [];
          const availablePositions = []; // Generate all possible positions
        ...
        createBrick(brickType, position, difficulty) {
          ...
          const brickComponent = brickNode.getComponent(_crd && EnhancedBrick === void 0 ? (_reportPossibleCrUseOfEnhancedBrick({
            error: Error()
        ...
        generateCorridorPattern(template) {
          const positions = []; // Create corridors with walls

          for (let row = 0; row < this.gridHeight; row++) {
            for (let col = 0; col < this.gridWidth; col++) {
              const isWall = col === 0 || col === this.gridWidth - 1 || row % 2 === 0 && col % 3 === 1;
...
//# sourceMappingURL=e730dd63ff0d44647ceddddf0e0d4fa9a387b0ba.js.map