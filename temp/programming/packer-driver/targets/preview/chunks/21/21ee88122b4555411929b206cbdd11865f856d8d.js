      ...
      _export("SlayTheSpireMapVisualizer", SlayTheSpireMapVisualizer = (_dec = ccclass('SlayTheSpireMapVisualizer'), _dec2 = property({
        ...
        setupMapContainers() {
          if (!this.mapContainer) {
            ...
            var widget = this.mapContainer.addComponent(Widget);
        ...
        createContainer(name, zIndex) {
          var container = new Node(name);
          container.setParent(this.mapContainer);
          container.setPosition(0, 0, zIndex);
          var transform = container.addComponent(UITransform);
        ...
        onMapTouchMove(event) {
          ...
          if (this.mapContainer) {
            ...
            var currentPos = this.mapContainer.getPosition();
        ...
        generateVisualMap(chapter) {
          ...
          var mapData = this.mapManager.generateChapterMap(chapter);
          var theme = this.getChapterTheme(chapter); // 设置章节主题
        ...
        applyChapterTheme(theme) {
          var config = this.chapterConfigs.get(theme);
        ...
        createThemedBackground(backgroundColor) {
          var bgNode = new Node('ThemedBackground');
          ...
          var bgTexture = this.createSolidColorTexture(backgroundColor);
        ...
        createFloorNodes(floor, nodeCount) {
          var floorY = -floor * 100; // 每层间距100像素

          var startX = -(nodeCount - 1) * 120 / 2; // 节点水平分布

          for (var i = 0; i < nodeCount; i++) {
            var nodeX = startX + i * 120;
            var nodePos = new Vec3(nodeX, floorY, 0); // 确定节点类型

            var nodeType = this.determineNodeType(floor, i); // 创建视觉节点

            var visualNode = this.createSingleVisualNode("node_" + floor + "_" + i, nodeType, nodePos, this.currentChapter, floor);
        ...
        createSingleVisualNode(id, type, position, chapter, floor) {
          var nodeUI = new Node("MapNode_" + id);
          nodeUI.setParent(this.nodeContainer);
          nodeUI.setPosition(position); // 添加UITransform

          var transform = nodeUI.addComponent(UITransform);
          transform.setContentSize(60, 60);
          transform.setAnchorPoint(0.5, 0.5); // 创建节点图标

          var iconSprite = this.createNodeIcon(nodeUI, type); // 添加按钮功能

          var button = nodeUI.addComponent(Button);
        ...
        createNodeIcon(parentNode, type) {
          ...
          var sprite = iconNode.addComponent(Sprite); // 根据节点类型设置图标

          var iconColor = this.getNodeTypeColor(type);
        ...
        createConnectionPaths() {
          ...
          this.visualNodes.forEach((node, nodeId) => {
            node.connections.forEach(connectionId => {
              var targetNode = this.visualNodes.get(connectionId);
        ...
        createPathLine(fromNode, toNode) {
          var pathNode = new Node("Path_" + fromNode.id + "_to_" + toNode.id);
          pathNode.setParent(this.pathContainer);
          var graphics = pathNode.addComponent(Graphics); // 获取主题配置

          var theme = this.getChapterTheme(this.currentChapter);
          var config = this.chapterConfigs.get(theme);
          var pathColor = (config == null ? void 0 : config.pathColor) || new Color(200, 200, 200, 255); // 绘制路径线条
        ...
        createParticleEffect(effectType) {
          var effectNode = new Node("ParticleEffect_" + effectType);
        ...
        onNodeClicked(nodeId) {
          var node = this.visualNodes.get(nodeId);
          if (!node) return;
        ...
        setCurrentPlayerPosition(nodeId) {
          ...
          if (this.currentPlayerPosition) {
            var oldNode = this.visualNodes.get(this.currentPlayerPosition);
          ...
          var newNode = this.visualNodes.get(nodeId);
        ...
        unlockConnectedNodes(node) {
          node.connections.forEach(connectionId => {
            var connectedNode = this.visualNodes.get(connectionId);
        ...
        centerMapOnCurrentNode() {
          ...
          var currentNode = this.visualNodes.get(this.currentPlayerPosition);
          if (!currentNode) return; // 将地图中心移动到当前节点

          var targetPos = new Vec3(-currentNode.position.x, -currentNode.position.y, 0);
...
//# sourceMappingURL=21ee88122b4555411929b206cbdd11865f856d8d.js.map