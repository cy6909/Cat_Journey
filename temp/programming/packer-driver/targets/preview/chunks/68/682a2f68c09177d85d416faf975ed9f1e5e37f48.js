    ...
    execute: function () {
      ...
      _export("EnhancedPaddleController", EnhancedPaddleController = (_dec = ccclass('EnhancedPaddleController'), _dec2 = property({
        ...
        update(dt) {
          ...
          if (this._rigidBody) {
            var vel = this._rigidBody.linearVelocity; // 检测是否有异常速度，如果有则清零并输出警告
        ...
        updatePaddlePosition(screenPos) {
          ...
          var clampedX = Math.max(leftBound, Math.min(rightBound, localPos.x)); // 🔒 直接设置位置，Y永远是-300
...
//# sourceMappingURL=682a2f68c09177d85d416faf975ed9f1e5e37f48.js.map