    ...
    execute: function () {
      ...
      _export("ShopManager", ShopManager = (_dec = ccclass('ShopManager'), _dec2 = property({
        ...
        createSpecialOffers() {
          ...
          var weekendEndTime = Date.now() + 7 * 24 * 60 * 60 * 1000;
        ...
        getShopItems(category) {
          var items = Array.from(this._shopItems.values());
        ...
        canPurchaseItem(itemId) {
          var item = this._shopItems.get(itemId);
          ...
          var purchaseRecord = this._purchaseRecords.get(itemId);
          ...
          if (item.currencyType && item.currencyAmount && this.monetizationManager) {
            var currentAmount = this.monetizationManager.getCurrency(item.currencyType);
        ...
        purchaseItem(itemId) {
          ...
          return _asyncToGenerator(function* () {
            var purchaseCheck = _this.canPurchaseItem(itemId);
            ...
            var item = _this._shopItems.get(itemId);

            var success = false;

            if (item.realPrice) {
              // 真实货币购买
              if (_this.monetizationManager) {
                var purchaseType = _this.getPurchaseTypeForItem(itemId);

                if (purchaseType) {
                  success = yield _this.monetizationManager.purchaseItem(purchaseType);
                }
              }
            } else if (item.currencyType && item.currencyAmount) {
              // 游戏币购买
              if (_this.monetizationManager && _this.monetizationManager.subtractCurrency(item.currencyType, item.currencyAmount)) {
                success = true;

                _this.grantItemRewards(item);
              }
            }

            if (success) {
              _this.recordPurchase(itemId);

              _this.savePurchaseRecords();
        ...
        isItemAvailable(itemId) {
          var endTime = this._limitedOffers.get(itemId);
        ...
        recordPurchase(itemId) {
          var record = this._purchaseRecords.get(itemId);
        ...
        resetDailyPurchasesIfNeeded() {
          ...
          for (var [itemId, record] of this._purchaseRecords.entries()) {
            var recordDate = new Date(record.purchaseTime).toDateString();
        ...
        generateDailyRefreshItems() {
          ...
          var availableItems = Array.from(this._shopItems.keys()).filter(itemId => {
            var item = this._shopItems.get(itemId);
        ...
        loadPurchaseRecords() {
          var data = sys.localStorage.getItem('shop_manager_data');
        ...
        addSpecialOffer(itemId, discountPercent, durationHours) {
          ...
          var endTime = Date.now() + durationHours * 60 * 60 * 1000;
...
//# sourceMappingURL=733cdeb531903aec6c70a1b25674759a2b7f77c6.js.map