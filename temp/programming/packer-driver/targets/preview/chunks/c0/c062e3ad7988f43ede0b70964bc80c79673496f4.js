      ...
      _export("EnhancedBrick", EnhancedBrick = (_dec = ccclass('EnhancedBrick'), _dec2 = property({
        ...
        onDestroyed(impactPosition) {
          ...
          var relicManager = (_crd && RelicManager === void 0 ? (_reportPossibleCrUseOfRelicManager({
            error: Error()
          }), RelicManager) : RelicManager).getInstance(); // Award score and experience
          ...
          this.applyDestructionEffects(impactPosition); // Drop experience orb for experience bricks
        ...
        createExplosion(center) {
          ...
          var allBricks = ((_this$node$parent = this.node.parent) == null ? void 0 : _this$node$parent.getComponentsInChildren(EnhancedBrick)) || [];

          for (var brick of allBricks) {
            if (brick === this) continue;
            var distance = Vec3.distance(center, brick.node.getWorldPosition());

            if (distance <= this.explosionRadius) {
              // Damage decreases with distance
              var damage = Math.max(1, Math.floor(3 * (1 - distance / this.explosionRadius)));
        ...
        triggerElectricChain() {
          var nearbyBricks = this.findNearbyBricks(this.electricChainDistance);

          for (var brick of nearbyBricks) {
        ...
        applyMagneticEffect(ballCollider) {
          var ballRigidBody = ballCollider.getComponent(RigidBody2D);
          ...
          var force = Vec2.multiplyScalar(new Vec2(), direction.x, direction.y, this.magneticForce);
        ...
        healPaddle() {
          ...
          var paddle = (_this$node$parent2 = this.node.parent) == null ? void 0 : _this$node$parent2.getComponentInChildren('EnhancedPaddleController');
        ...
        findNearbyBricks(radius) {
          ...
          var allBricks = ((_this$node$parent3 = this.node.parent) == null ? void 0 : _this$node$parent3.getComponentsInChildren(EnhancedBrick)) || [];
          var nearby = [];

          for (var brick of allBricks) {
            if (brick === this) continue;
            var distance = Vec3.distance(this.node.getWorldPosition(), brick.node.getWorldPosition());
        ...
        updateVisualState() {
          ...
          var healthRatio = this.health / this._maxHealth;

          var currentColor = this._originalColor.clone(); // Darken based on damage
...
//# sourceMappingURL=c062e3ad7988f43ede0b70964bc80c79673496f4.js.map