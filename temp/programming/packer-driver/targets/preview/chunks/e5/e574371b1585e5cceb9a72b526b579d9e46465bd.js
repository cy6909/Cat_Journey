    ...
    execute: function () {
      ...
      _export("MapManager", MapManager = (_dec = ccclass('MapManager'), _dec2 = property({
        ...
        generateChapterMap(chapter) {
          ...
          var startNodes = this.getNodesByType(NodeType.START);
        ...
        generateMapNodes() {
          ...
          for (var floor = 0; floor < this.floorsPerChapter; floor++) {
        ...
        generateFloorNodes(floor) {
          var nodeCount = this.getNodeCountForFloor(floor);
          ...
          if (floor === 0) {
            ...
            // Boss floor
            var bossTypes = this._chapterBossTypes.get(this._currentChapter) || [];
            var randomBoss = bossTypes[Math.floor(Math.random() * bossTypes.length)];
            var bossNode = this.createNode("boss_" + floor, NodeType.BOSS, floor, 0);
            bossNode.bossData = {
              bossType: randomBoss,
        ...
        generateRegularFloor(floor, nodeCount) {
          var nodesToCreate = []; // Determine node types for this floor

          for (var i = 0; i < nodeCount; i++) {
            var nodeType = this.selectRandomNodeType();
          ...
          for (var _i = 0; _i < nodeCount; _i++) {
            var nodeId = nodesToCreate[_i] + "_" + floor + "_" + _i;
            this.createNodeByType(nodeId, nodesToCreate[_i], floor, _i);
          }
        }

        createNodeByType(nodeId, nodeType, floor, position) {
          var node = this.createNode(nodeId, nodeType, floor, position);
        ...
        createCombatNode(nodeId, floor, position) {
          var node = this.createNode(nodeId, NodeType.COMBAT, floor, position);
          this.setupCombatNode(node, floor);
        }

        setupCombatNode(node, floor) {
          var baseDifficulty = 1 + (this._currentChapter - 1) * 0.5 + floor * 0.1;
          node.combatData = {
        ...
        setupEliteNode(node, floor) {
          var randomElite = this._availableEliteTypes[Math.floor(Math.random() * this._availableEliteTypes.length)];
        ...
        setupEventNode(node, floor) {
          var events = this.getAvailableEvents(this._chapterTheme, floor);
        ...
        setupTreasureNode(node, floor) {
          var treasureTypes = ['relic', 'currency', 'upgrade_material'];
          var randomType = treasureTypes[Math.floor(Math.random() * treasureTypes.length)];
          node.treasureData = {
        ...
        generateConnections() {
          var nodesByFloor = this.groupNodesByFloor();

          for (var floor = 0; floor < this.floorsPerChapter - 1; floor++) {
            var currentFloorNodes = nodesByFloor.get(floor) || [];
            var nextFloorNodes = nodesByFloor.get(floor + 1) || [];
        ...
        connectFloors(currentFloor, nextFloor, _floorIndex) {
          ...
          for (var currentNode of currentFloor) {
            var connectionCount = Math.min(nextFloor.length, Math.floor(Math.random() * 3) + 1); // Choose random nodes to connect to, preferring nearby positions

            var targetIndices = this.selectConnectionTargets(currentFloor.indexOf(currentNode), nextFloor.length, connectionCount);

            for (var targetIndex of targetIndices) {
              var targetNode = nextFloor[targetIndex];
        ...
        selectConnectionTargets(sourceIndex, targetCount, connectionCount) {
          var targets = [];
          var sourcePosition = sourceIndex / Math.max(1, targetCount - 1); // Normalize to 0-1
          ...
          for (var i = 0; i < connectionCount; i++) {
            ...
            if (i === 0) {
              ...
              var range = Math.min(2, Math.floor(targetCount / 2));
              var centerIndex = Math.round(sourcePosition * (targetCount - 1));
              var minIndex = Math.max(0, centerIndex - range);
              var maxIndex = Math.min(targetCount - 1, centerIndex + range);
        ...
        ensureNodesReachable(currentFloor, nextFloor) {
          var reachableNodes = new Set(); // Find all reachable nodes

          for (var currentNode of currentFloor) {
            for (var connectionId of currentNode.connections) {
              reachableNodes.add(connectionId);
            }
          } // Connect unreachable nodes to random current floor nodes


          for (var nextNode of nextFloor) {
            if (!reachableNodes.has(nextNode.id)) {
              var randomCurrentNode = currentFloor[Math.floor(Math.random() * currentFloor.length)];
        ...
        createMapVisuals() {
          ...
          for (var [_nodeId, node] of this._mapNodes) {
        ...
        createNodeVisual(node) {
          ...
          var nodeVisual = instantiate(this.mapNodePrefab);
          var sprite = nodeVisual.getComponent(Sprite);
          var label = nodeVisual.getComponentInChildren(Label);
          var button = nodeVisual.getComponent(Button);
        ...
        createConnectionVisuals() {
          ...
          for (var [_nodeId, node] of this._mapNodes) {
            for (var connectionId of node.connections) {
              var targetNode = this._mapNodes.get(connectionId);

              if (!targetNode) continue;
              var line = instantiate(this.connectionLinePrefab);
              line.setParent(this.node); // Position and scale line between nodes

              var startPos = node.position;
              var endPos = targetNode.position;
              var midPos = Vec3.lerp(new Vec3(), startPos, endPos, 0.5);
              var distance = Vec3.distance(startPos, endPos);
              line.setPosition(midPos);
              var transform = line.getComponent(UITransform);
        ...
        onNodeClicked(nodeId) {
          var node = this._mapNodes.get(nodeId);
        ...
        selectRandomNodeType() {
          var totalWeight = Array.from(this._nodeTypeDistribution.values()).reduce((sum, weight) => sum + weight, 0);
          var random = Math.random() * totalWeight;

          for (var [nodeType, weight] of this._nodeTypeDistribution.entries()) {
            random -= weight;
        ...
        groupNodesByFloor() {
          var floorGroups = new Map();

          for (var node of this._mapNodes.values()) {
        ...
        updateNodeAvailability() {
          ...
          var currentNode = this._mapNodes.get(this._currentNodeId);

          if (currentNode) {
            for (var connectionId of currentNode.connections) {
              var connectedNode = this._mapNodes.get(connectionId);
        ...
        updateAllNodeVisuals() {
          for (var [nodeId, node] of this._mapNodes) {
            var visual = this._nodeVisuals.get(nodeId);
        ...
        updateNodeVisualState(visual, node) {
          var sprite = visual.getComponent(Sprite);
          if (!sprite) return;
        ...
        clearMapVisuals() {
          // Clear node visuals
          for (var visual of this._nodeVisuals.values()) {
            visual.destroy();
          }
          ...
          for (var line of this._connectionLines) {
            line.destroy();
          }
        ...
        getCurrentNodeType() {
          var currentNode = this.getNodeById(this._currentNodeId);
...
//# sourceMappingURL=e574371b1585e5cceb9a72b526b579d9e46465bd.js.map