      ...
      _export("LevelTransitionManager", LevelTransitionManager = (_dec = ccclass('LevelTransitionManager'), _dec2 = property({
        ...
        setupTransitionLayers() {
          if (!this.transitionContainer) {
            ...
            var widget = this.transitionContainer.addComponent(Widget);
        ...
        createLayer(name, zIndex) {
          ...
          var transform = layer.addComponent(UITransform);
        ...
        startLevelTransition(fromLevel, toLevel, chapter, nodeData) {
          var _this = this;

          return _asyncToGenerator(function* () {
            if (_this.isTransitioning) {
              console.warn('Transition already in progress');
              return _this.currentTransition || Promise.resolve();
            }

            _this.isTransitioning = true;
            console.log("Starting transition: " + fromLevel + " -> " + toLevel + " (Chapter: " + chapter + ")");
            _this.currentTransition = _this.executeTransition(fromLevel, toLevel, chapter, nodeData);
        ...
        executeTransition(fromLevel, toLevel, chapter, nodeData) {
          var _this2 = this;

          return _asyncToGenerator(function* () {
            // 显示过场容器
            _this2.transitionContainer.active = true; // 第一阶段：淡出当前场景

            yield _this2.fadeOutCurrentScene(); // 第二阶段：显示过场内容

            yield _this2.showTransitionContent(toLevel, chapter, nodeData); // 第三阶段：预加载资源

            yield _this2.preloadLevelAssets(toLevel); // 第四阶段：过场动画

            yield _this2.playTransitionAnimation(toLevel, chapter); // 第五阶段：淡入新场景

            yield _this2.fadeInNewScene(); // 隐藏过场容器

            _this2.transitionContainer.active = false;
        ...
        fadeOutCurrentScene() {
          ...
          return _asyncToGenerator(function* () {
            return new Promise(resolve => {
              // 创建黑色遮罩
              var fadeOverlay = _this3.createFadeOverlay(); // 淡入黑色遮罩
        ...
        createFadeOverlay() {
          var overlay = new Node('FadeOverlay');
          overlay.setParent(this.backgroundLayer);
          var transform = overlay.addComponent(UITransform);
          var screenSize = view.getVisibleSize();
          transform.setContentSize(screenSize.width, screenSize.height);
          transform.setAnchorPoint(0.5, 0.5);
          var sprite = overlay.addComponent(Sprite);
        ...
        showTransitionContent(levelType, chapter, nodeData) {
          var _this4 = this;

          return _asyncToGenerator(function* () {
            var chapterTheme = _this4.chapterThemes.get(chapter);

            if (!chapterTheme) return; // 清理之前的内容

            _this4.contentLayer.removeAllChildren(); // 创建章节背景


            _this4.createChapterBackground(chapterTheme); // 创建关卡类型显示


            _this4.createLevelTypeDisplay(levelType, chapterTheme); // 创建加载文本


            _this4.createLoadingText(chapterTheme.loadingText); // 创建章节信息


            _this4.createChapterInfo(chapterTheme); // 添加特效


            if (_this4.enableParticleEffects) {
              _this4.createTransitionParticles(chapterTheme);
            }
          })();
        }

        createChapterBackground(theme) {
          var bgNode = new Node('ChapterBackground');
          bgNode.setParent(this.contentLayer);
          var transform = bgNode.addComponent(UITransform);
          var screenSize = view.getVisibleSize();
          transform.setContentSize(screenSize.width, screenSize.height);
          transform.setAnchorPoint(0.5, 0.5);
        ...
        createLevelTypeDisplay(levelType, theme) {
          var displayNode = new Node('LevelTypeDisplay');
          displayNode.setParent(this.contentLayer);
          displayNode.setPosition(0, 100, 0);
          var transform = displayNode.addComponent(UITransform);
          transform.setContentSize(300, 80);
          transform.setAnchorPoint(0.5, 0.5);
          var label = displayNode.addComponent(Label);
        ...
        createLoadingText(loadingText) {
          var textNode = new Node('LoadingText');
          textNode.setParent(this.contentLayer);
          textNode.setPosition(0, -100, 0);
          var transform = textNode.addComponent(UITransform);
        ...
        createChapterInfo(theme) {
          ...
          var subtitleNode = new Node('ChapterSubtitle');
        ...
        createTransitionParticles(theme) {
          theme.particleEffects.forEach((effectType, index) => {
            var particleNode = new Node("Particle_" + effectType);
        ...
        preloadLevelAssets(levelType) {
          var _this5 = this;

          return _asyncToGenerator(function* () {
            var config = _this5.transitionConfigs.get(levelType);
        ...
        playTransitionAnimation(levelType, chapter) {
          var _this6 = this;

          return _asyncToGenerator(function* () {
            var config = _this6.transitionConfigs.get(levelType);

            var theme = _this6.chapterThemes.get(chapter);
            ...
            switch (theme.transitionType) {
              case TransitionType.FADE_TO_BLACK:
                yield _this6.playFadeTransition();
                break;

              case TransitionType.SLIDE_LEFT:
                yield _this6.playSlideTransition('left');
                break;

              case TransitionType.SPIRAL:
                yield _this6.playSpiralTransition();
                break;
        ...
        playFadeTransition() {
          ...
          return _asyncToGenerator(function* () {
            return new Promise(resolve => {
              var duration = _this7.transitionDuration; // 创建渐变效果

              tween(_this7.contentLayer).to(duration * 0.3, {
        ...
        playSlideTransition(direction) {
          var _this8 = this;

          return _asyncToGenerator(function* () {
            return new Promise(resolve => {
              var screenSize = view.getVisibleSize();
              var startX = direction === 'left' ? screenSize.width : -screenSize.width;

              _this8.contentLayer.setPosition(startX, 0, 0);

              tween(_this8.contentLayer).to(_this8.transitionDuration, {
        ...
        playSpiralTransition() {
          var _this9 = this;

          return _asyncToGenerator(function* () {
            return new Promise(resolve => {
              var spiralTween = tween(_this9.contentLayer).to(_this9.transitionDuration, {
                eulerAngles: new Vec3(0, 0, 720),
                // 旋转两圈
                scale: new Vec3(0.1, 0.1, 1)
              }).to(_this9.transitionDuration * 0.5, {
        ...
        fadeInNewScene() {
          ...
          return _asyncToGenerator(function* () {
            return new Promise(resolve => {
              // 淡出过场遮罩
              var fadeOverlay = _this10.backgroundLayer.getChildByName('FadeOverlay');

              if (fadeOverlay) {
                var sprite = fadeOverlay.getComponent(Sprite);
        ...
        quickTransition(toLevel) {
          var _this11 = this;

          return _asyncToGenerator(function* () {
            if (_this11.isTransitioning) return;
            _this11.isTransitioning = true;

            try {
              _this11.transitionContainer.active = true; // 简单的淡入淡出

              var overlay = _this11.createFadeOverlay();

              var sprite = overlay.getComponent(Sprite); // 淡入黑色
              ...
              _this11.createLoadingText("\u8FDB\u5165" + _this11.getLevelTypeDisplayName(toLevel) + "..."); // 短暂等待
              ...
              _this11.transitionContainer.active = false;
            } finally {
              _this11.isTransitioning = false;
            }
          })();
        } // 章节切换特殊过场


        chapterTransition(fromChapter, toChapter) {
          var _this12 = this;

          return _asyncToGenerator(function* () {
            if (_this12.isTransitioning) return;
            console.log("Chapter transition: " + fromChapter + " -> " + toChapter);

            var fromTheme = _this12.chapterThemes.get(fromChapter);

            var toTheme = _this12.chapterThemes.get(toChapter);

            if (!fromTheme || !toTheme) return;
            _this12.isTransitioning = true;

            try {
              _this12.transitionContainer.active = true; // 创建章节过渡动画

              yield _this12.playChapterTransitionSequence(fromTheme, toTheme);
              _this12.transitionContainer.active = false;
            } finally {
              _this12.isTransitioning = false;
            }
          })();
        }

        playChapterTransitionSequence(fromTheme, toTheme) {
          var _this13 = this;

          return _asyncToGenerator(function* () {
            // 第一阶段：展示前章节结束
            yield _this13.showChapterEnd(fromTheme); // 第二阶段：过渡动画

            yield _this13.playChapterBridgeAnimation(); // 第三阶段：展示新章节开始

            yield _this13.showChapterBegin(toTheme);
          })();
        }

        showChapterEnd(theme) {
          var _this14 = this;

          return _asyncToGenerator(function* () {
            // 显示章节完成信息
            var endNode = new Node('ChapterEnd');
            endNode.setParent(_this14.contentLayer);
            var label = endNode.addComponent(Label);
        ...
        showChapterBegin(theme) {
          var _this15 = this;

          return _asyncToGenerator(function* () {
            // 显示新章节开始信息
            _this15.createChapterInfo(theme);
...
//# sourceMappingURL=ef0d5b034015d3f86b1fb60781962a8cfe296f77.js.map