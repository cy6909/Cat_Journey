    ...
    execute: function () {
      ...
      _export("UltraSimpleExperienceOrb", UltraSimpleExperienceOrb = (_dec = ccclass('UltraSimpleExperienceOrb'), _dec(_class = (_class2 = class UltraSimpleExperienceOrb extends Component {
        ...
        update(dt) {
          ...
          var currentWorldPos = this.node.getWorldPosition(); // 检查是否碰到挡板

          if (this._paddleNode && this._paddleNode.isValid) {
            var paddleWorldPos = this._paddleNode.getWorldPosition(); // 简单的矩形碰撞检测


            var xDiff = Math.abs(currentWorldPos.x - paddleWorldPos.x);
            var yDiff = Math.abs(currentWorldPos.y - paddleWorldPos.y); // 挡板宽度约80，高度约20；经验球约20x20
          ...
          var currentLocalPos = this.node.position;
          var newPos = new Vec3(currentLocalPos.x, currentLocalPos.y + this._velocity.y * dt, currentLocalPos.z); // 底部边界检查 - 掉出屏幕则销毁
