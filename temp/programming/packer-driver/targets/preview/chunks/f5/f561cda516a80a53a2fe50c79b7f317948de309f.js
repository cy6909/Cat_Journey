    ...
    execute: function () {
      ...
      _export("DifficultyCalculator", DifficultyCalculator = class DifficultyCalculator {
        ...
        static calculateDifficulty(level) {
          ...
          var baseHealth = 1 + Math.floor(level / 5); // 特殊砖概率: 每关+2%, 上限50%

          var specialBrickChance = Math.min(level * 0.02, 0.5); // 减益砖概率: 每关+1.5%, 上限30%

          var harmfulBrickChance = Math.min(level * 0.015, 0.3); // 有益砖概率: 固定5%

          var beneficialBrickChance = 0.05; // 布局类型: <10关用Normal, >=10关用Special

          var layoutType = level < 10 ? 'normal' : 'special'; // 密度: 基础60% + 每关+2%, 上限90%

          var density = 0.6 + Math.min(level * 0.02, 0.3); // 网格尺寸: 根据关卡递增
        ...
        static selectBrickTypeByWeight(types, weights) {
          ...
          var totalWeight = weights.reduce((sum, w) => sum + w, 0); // 随机值

          var random = Math.random() * totalWeight; // 选择类型
